...
public class SmsDatabaseHelper extends SQLiteOpenHelper {
    ...
    private static final String[] COLUMN_NAMES = {"SmsID", "Name", "Number", "messageDate", "messageTime", "Message", "messageStatus"};
    ...
    public int addSms(Sms sms) {
        ...
        row.put(this.COLUMN_NAMES[3], sms.messageDate);
        row.put(this.COLUMN_NAMES[4], sms.messageTime);
        row.put(this.COLUMN_NAMES[5], sms.message);
    ...
    public ArrayList<Sms> getPendingSmsList() {
        ...
        Cursor result = db.rawQuery("SELECT * FROM Messages WHERE messageStatus ='Pending' ORDER BY SmsID DESC", null);
    ...
    public int retrieveSmsID(Sms sms) {
        ...
        String whereClause = "Name = '" + sms.name + "' AND Number = '" + sms.number + "' AND messageDate= '" + sms.messageDate + "' AND messageTime= '" + sms.messageTime + "' AND message = '" + sms.message + "'";
